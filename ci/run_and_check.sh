...
# Script to launch spark-pac4j-demo and verify it works
# Usage: ./run_and_check.sh

set -e  # Stop script on error

echo "🚀 Starting spark-pac4j-demo..."
...
# Start Spark in background using exec plugin
...
if [ "$HTTP_CODE" = "200" ] && [ "$CAS_TEST_PASSED" = "true" ] && [ "$CAS_AUTH_PASSED" = "true" ]; then
    echo "🎉 spark-pac4j-demo test completed successfully!"
    ...
    echo "💥 spark-pac4j-demo test failed!"
    if [ "$HTTP_CODE" != "200" ]; then
